...
type DesignTokens = {
  colors: {
    ...
    neutral: { "900": string; "700": string; "500": string; "200": string; "50": string };
    accent: { ai: string };
  ...
  shadows: Record<string, ShadowToken | string>;
...
type PluginMessage = CreateDraftMessage | FetchRequestMessage;
...
async function handleFetchRequest(message: FetchRequestMessage): Promise<void> {
  try {
...
function renderDesignPlan(job: DesignJob, designPlan: DesignPlan, tokens: DesignTokens): FrameNode {
  ...
  for (const block of blocksToRender) {
    frame.appendChild(renderContentBlock(block as Record<string, unknown>, tokens));
  }
...
function getDefaultBlocksForPattern(layoutPattern: string): Array<Record<string, unknown>> {
...
function sortBlocksForDashboard(blocks: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  ...
  return [...blocks].sort((a, b) => {
    const ap = typePriority[typeof a.type === "string" ? a.type : ""] ?? 99;
    const bp = typePriority[typeof b.type === "string" ? b.type : ""] ?? 99;
    return ap - bp;
  });
}

function renderContentBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function renderPageHeaderBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function renderHeroSectionBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function renderMetricCardsBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
  ...
  for (const item of cardItems) {
    const label = typeof item === "object" && item !== null ? String((item as Record<string, unknown>).label || "Metric") : "Metric";
    const value = typeof item === "object" && item !== null ? String((item as Record<string, unknown>).value || "—") : "—";
    const desc  = typeof item === "object" && item !== null ? String((item as Record<string, unknown>).description || "") : "";
...
function renderCardGridBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
  ...
  for (let i = 0; i < gridItems.length; i += rowSize) {
    ...
    for (const item of rowItems) {
      const label = typeof item === "string" ? item : String((item as Record<string, unknown>).label || "Card");
      const desc  = typeof item === "object" && item !== null ? String((item as Record<string, unknown>).description || "") : "";
...
function renderTableSectionBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
  const title = blockStr(block, "title") || "Table";
  const columns = blockArr(block, "columns");
  const colNames = columns.length > 0
    ? columns.map(c => typeof c === "string" ? c : String((c as Record<string, unknown>).label || (c as Record<string, unknown>).name || "Col"))
...
function buildTableRow(
  name: string, values: string[], colWidth: number, colGap: number,
  textStyleName: string, weight: string, color: string, tokens: DesignTokens
): FrameNode {
...
function buildTableDivider(colorHex: string): FrameNode {
...
function renderFormSectionBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
  const title = blockStr(block, "title") || "Form";
  const fields = blockArr(block, "fields");
  const fieldNames = fields.length > 0
    ? fields.slice(0, 6).map(f => typeof f === "string" ? f : String((f as Record<string, unknown>).label || (f as Record<string, unknown>).name || "Field"))
...
function renderListSectionBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
  ...
  for (const item of listItems) {
    const label = typeof item === "string" ? item : String((item as Record<string, unknown>).label || (item as Record<string, unknown>).name || "—");
    const desc  = typeof item === "object" && item !== null ? String((item as Record<string, unknown>).description || "") : "";
...
function renderEmptyStateBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function renderLoadingStateBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function renderErrorStateBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function renderSuccessStateBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function renderGenericContentBlock(block: Record<string, unknown>, tokens: DesignTokens): FrameNode {
...
function createDesignPlanHeader(issueKey: string, title: string, objective: string, targetUser: string, layoutPattern: string, tokens: DesignTokens): FrameNode {
...
function createBlockContainer(name: string, tokens: DesignTokens): FrameNode {
  const container = figma.createFrame();
  container.name = name;
  container.layoutMode = "VERTICAL";
  container.primaryAxisSizingMode = "AUTO";
...
function createGenericMetricCard(label: string, value: string, description: string, tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  card.name = label;
  card.layoutMode = "VERTICAL";
  card.primaryAxisSizingMode = "AUTO";
...
function blockStr(block: Record<string, unknown>, key: string): string {
  const v = block[key];
  return typeof v === "string" && v.trim() ? v.trim() : "";
}

function blockArr(block: Record<string, unknown>, key: string): unknown[] {
  const v = block[key];
...
function createDraftFrame(job: DesignJob, tokens: DesignTokens): FrameNode {
  const issueKey = textOrFallback(job.issueKey, "No issue key");
...
function createDashboardDraft(job: DesignJob, tokens: DesignTokens): FrameNode {
  const issueKey = textOrFallback(job.issueKey, "No issue key");
...
function createDashboardNav(title: string, badgeLabel: string, tokens: DesignTokens): FrameNode {
...
function createDashboardSidebar(tokens: DesignTokens): FrameNode {
...
function createDashboardMain(job: DesignJob, tokens: DesignTokens): FrameNode {
  ...
  const pageTitle = createTokenText(
    textOrFallback(job.briefTitle, "Dashboard Overview"),
    tokens, "h1", "bold", tokens.colors.neutral["900"]
  );
  pageTitle.layoutAlign = "STRETCH";
  main.appendChild(pageTitle);

  const subtitle = createTokenText(
    truncateText(textOrFallback(job.objective, "Monitor key metrics at a glance."), 120),
    tokens, "body", "regular", tokens.colors.neutral["500"]
  );
  subtitle.layoutAlign = "STRETCH";
...
function createMetricCardsRow(tokens: DesignTokens): FrameNode {
...
function createMetricCard(label: string, value: string, tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  card.name = label;
  card.layoutMode = "VERTICAL";
  card.primaryAxisSizingMode = "AUTO";
...
function createChartsRow(tokens: DesignTokens): FrameNode {
  const charts = [
...
function createChartPlaceholder(title: string, width: number, tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  card.name = title;
  card.layoutMode = "VERTICAL";
  card.primaryAxisSizingMode = "AUTO";
...
function createDashboardTablePlaceholder(tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  ...
  for (const col of cols) {
    ...
    if (col !== "Post") cell.resize(100, cell.height);
  ...
  for (let r = 0; r < 2; r++) {
    ...
    for (let c = 0; c < cells.length; c++) {
      ...
      if (c !== 0) cell.resize(100, cell.height);
...
function getConversationDashboardSubtype(job: DesignJob): "conversation" | "analytics" {
  const text = [
    job.briefTitle,
    job.title,
    job.objective,
  ...
  const subtype: "conversation" | "analytics" = matched.length > 0 ? "conversation" : "analytics";
  console.log(`[Dashboard] selected dashboard subtype: ${subtype}-dashboard`);
  return subtype;
}

function createConversationDashboardSidebar(bodyHeight: number, tokens: DesignTokens): FrameNode {
  const sidebar = figma.createFrame();
  sidebar.name = "Sidebar";
  sidebar.layoutMode = "VERTICAL";
  sidebar.primaryAxisSizingMode = "FIXED";
...
function createConversationDashboardMain(job: DesignJob, bodyHeight: number, tokens: DesignTokens): FrameNode {
  ...
  const pageTitle = createTokenText(
    textOrFallback(job.briefTitle, "Conversation Dashboard"),
    tokens, "h1", "bold", tokens.colors.neutral["900"]
  );
  pageTitle.layoutAlign = "STRETCH";
  main.appendChild(pageTitle);
  ...
  const subtitle = createTokenText(
    truncateText(textOrFallback(job.objective, "Monitor AI conversation performance and handoff queue."), 120),
    tokens, "body", "regular", tokens.colors.neutral["500"]
  );
  subtitle.layoutAlign = "STRETCH";
  main.appendChild(subtitle);
...
function createConversationMetricsRow(tokens: DesignTokens): FrameNode {
  // 4 × 277px + 3 × 16px gap = 1156px
  const metrics: Array<{ label: string; value: string; trend: string; up: boolean }> = [
...
function createConversationMetricCard(label: string, value: string, trend: string, trendPositive: boolean, tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  card.name = label;
  card.layoutMode = "VERTICAL";
  card.primaryAxisSizingMode = "AUTO";
...
function createConversationMiddleRow(tokens: DesignTokens): FrameNode {
...
function createRecentConversationsSection(tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  ...
  const conversations: Array<{ handle: string; message: string; confidence: number; status: string; time: string }> = [
...
function createAiQualitySection(tokens: DesignTokens): FrameNode {
  ...
  const bands: Array<{ label: string; count: string; pct: number; color: string }> = [
...
function createConversationBottomRow(tokens: DesignTokens): FrameNode {
...
function createHandoffQueueSection(tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  ...
  const queueItems: Array<{ handle: string; reason: string; wait: string; priority: "high" | "medium" }> = [
...
function createSyncStatusSection(tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  ...
  const statuses: Array<{ label: string; detail: string; value: string; ok: boolean }> = [
...
function createStatePreviewsRow(tokens: DesignTokens): FrameNode {
...
function createLoadingStateCard(tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
...
function createEmptyStateCard(tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
...
function createErrorStateCard(tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
...
function createStatusPill(text: string, bgHex: string, textHex: string, tokens: DesignTokens): FrameNode {
...
function createInitialsAvatar(handle: string, bgHex: string, tokens: DesignTokens): FrameNode {
  const avatar = figma.createFrame();
  avatar.name = "avatar";
...
function createOnboardingDraft(job: DesignJob, tokens: DesignTokens): FrameNode {
  const issueKey = textOrFallback(job.issueKey, "No issue key");
...
function createOnboardingStepper(tokens: DesignTokens): FrameNode {
...
function createOnboardingCard(job: DesignJob, tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  ...
  const heading = createTokenText(
    textOrFallback(job.briefTitle, "Welcome — let's get you set up"),
    tokens, "h1", "bold", tokens.colors.neutral["900"]
  );
  heading.layoutAlign = "STRETCH";
  card.appendChild(heading);

  const description = createTokenText(
    truncateText(textOrFallback(job.objective, "Complete a few quick steps to activate your account."), 160),
    tokens, "body", "regular", tokens.colors.neutral["700"]
  );
  ...
  if (job.uxNotes || job.acceptanceCriteria) {
    card.appendChild(createNoteCard(
      truncateText(textOrFallback(job.uxNotes || job.acceptanceCriteria, ""), 200),
      tokens
    ));
  }
...
function createOnboardingFields(tokens: DesignTokens): FrameNode {
...
function createDesignSystemDraft(job: DesignJob, tokens: DesignTokens): FrameNode {
  const issueKey = textOrFallback(job.issueKey, "No issue key");
...
function createDesignSystemHeader(title: string, issueKey: string, targetUser: string, objective: string, tokens: DesignTokens): FrameNode {
  const header = figma.createFrame();
  header.name = "Header";
  header.layoutMode = "VERTICAL";
  header.primaryAxisSizingMode = "AUTO";
...
function createColorSystemSection(tokens: DesignTokens): FrameNode {
...
function createTypographySection(tokens: DesignTokens): FrameNode {
...
function createSpacingSection(tokens: DesignTokens): FrameNode {
  const rows = tokens.spacing.map((value) => createSpacingRow(value, tokens));
  return createSectionCard("Spacing Scale", rows, tokens);
}

function createRadiusShadowSection(tokens: DesignTokens): FrameNode {
...
function createComponentFoundationSection(tokens: DesignTokens): FrameNode {
...
function createStateRulesSection(tokens: DesignTokens): FrameNode {
...
function createNamingConventionSection(tokens: DesignTokens): FrameNode {
...
function createAiUsageRulesSection(tokens: DesignTokens): FrameNode {
...
function createSectionCard(title: string, children: SceneNode[], tokens: DesignTokens): FrameNode {
  const card = figma.createFrame();
  card.name = title;
  card.layoutMode = "VERTICAL";
  card.primaryAxisSizingMode = "AUTO";
...
function createTextSectionCard(title: string, body: string | undefined, tokens: DesignTokens): FrameNode {
...
function createNoteCard(body: string, tokens: DesignTokens): FrameNode {
...
function createModeBadge(label: string, tokens: DesignTokens): FrameNode {
...
function ensureMinimumFrameHeight(frame: FrameNode, minimumHeight: number): void {
...
function createSwatch(name: string, hex: string, tokens: DesignTokens): FrameNode {
...
function createTypographyRow(name: string, fontSizeValue: string, weight: string, tokens: DesignTokens): FrameNode {
...
function createSpacingRow(value: number, tokens: DesignTokens): FrameNode {
...
function createCodePill(text: string, tokens: DesignTokens): FrameNode {
  const pill = figma.createFrame();
  pill.name = text;
  pill.layoutMode = "HORIZONTAL";
  pill.primaryAxisSizingMode = "AUTO";
...
function createRadiusExample(name: string, radius: number, tokens: DesignTokens): FrameNode {
...
function createShadowExample(name: string, tokenName: string, tokens: DesignTokens): FrameNode {
...
function createComponentPlaceholder(name: string, tokens: DesignTokens): FrameNode {
...
function createStateCard(name: string, tokens: DesignTokens): FrameNode {
...
function createBulletRow(text: string, tokens: DesignTokens): FrameNode {
...
function createVisualTile(name: string, width: number, height: number, tokens: DesignTokens): FrameNode {
...
function createHorizontalGroup(children: SceneNode[], itemSpacing: number): FrameNode {
...
function createRow(name: string, itemSpacing: number): FrameNode {
  const row = figma.createFrame();
  row.name = name;
  row.layoutMode = "HORIZONTAL";
  row.primaryAxisSizingMode = "AUTO";
...
function createPrimaryButton(label: string, tokens: DesignTokens): FrameNode {
...
function createTokenText(characters: string, tokens: DesignTokens, styleName: string, weightFallback: string, colorHex: string): TextNode {
...
function findNextFramePosition(): { x: number; y: number } {
  let rightmost = 0;
...
function jobSearchText(job: DesignJob): string {
  return [
    job.briefTitle,
    job.objective,
    job.figmaInstruction,
...
function isDesignSystemJob(job: DesignJob): boolean {
...
function isDashboardJob(job: DesignJob): boolean {
  return matchesKeywords(jobSearchText(job), dashboardKeywords);
}

function isOnboardingJob(job: DesignJob): boolean {
  return matchesKeywords(jobSearchText(job), onboardingKeywords);
}

function normalizeTokens(tokens: DesignTokens | undefined): DesignTokens {
  if (!tokens) {
...
function normalizeJob(job: DesignJob): DesignJob {
  ...
  const arrStr = (v: unknown): string | undefined => {
    if (Array.isArray(v)) {
      const joined = (v as unknown[]).filter((x): x is string => typeof x === "string").join(", ");
...
function textStyle(tokens: DesignTokens, name: string): TextStyleToken {
...
function colorSwatchTokens(tokens: DesignTokens): Array<{ name: string; hex: string }> {
  return [
...
function spacing(tokens: DesignTokens, index: number): number {
...
function labelForTypography(key: string): string {
  if (key === "h1") return "H1";
  if (key === "h2") return "H2";
...
function parseSize(size: string, fallback: number): number {
...
function setPadding(frame: FrameNode, value: number): void {
...
function frameLike(frame: FrameNode): FrameNode {
  return frame;
}

function applyShadow(frame: FrameNode, tokens: DesignTokens, tokenName: string): void {
...
function stateColor(name: string, tokens: DesignTokens): string {
